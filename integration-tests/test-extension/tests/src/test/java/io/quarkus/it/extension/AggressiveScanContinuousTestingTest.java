...
public class AggressiveScanContinuousTestingTest {
    ...
    public void testDevConsoleManagerRaceCondition() throws Exception {
        ...
        System.out.println("\n>>> Starting EXTREME restart hammering...");
        System.out.println(">>> This will trigger 5 rapid restarts with aggressive scanning");
        System.out.println(">>> Each restart will trigger scans in pre/post restart hooks");

        try {
            // Run test MULTIPLE TIMES to increase probability
            // Race condition is timing-dependent and may not occur every time
            int attempts = 20; // Try 20 times
            boolean crashed = false;

            System.out.println(">>> Running " + attempts + " attempts to trigger race condition...");

            for (int attempt = 1; attempt <= attempts && !crashed; attempt++) {
                System.out.println("\n=== ATTEMPT " + attempt + "/" + attempts + " ===");

                // Trigger 3 rapid restarts per attempt
                for (int i = 1; i <= 3; i++) {
                    String from = (attempt == 1 && i == 1) ? "third" : "v" + ((attempt - 1) * 3 + i - 1);
                    String to = "v" + ((attempt - 1) * 3 + i);

                    System.out.println("[Attempt " + attempt + ", Restart " + i + "/3] " + from + " → " + to);
                    TEST.modifyResourceFile("application.properties", s -> s.replace(from, to));

                    // EXTREMELY short delay - maximize overlap
                    if (i < 3) {
                        Thread.sleep(20); // Only 20ms between restarts!
                    }
                }
